...
public class PromptGenerator {
    ...
    public static final String BGL_TEMPLATE_AWARE_FINAL_PROMPT = """
            ...
            Message meaning has higher priority than severity words. FATAL, ERROR, failed, interrupt, exception, ASSERT are not enough alone.
            
            Return 0 for known normal/non-alert patterns:
            - corrected or detected and corrected errors
            - diagnostic/register/context messages such as exception syndrome register, machine state register, instruction address, data address, generating core
            - program interrupt: privileged instruction, trap instruction, illegal instruction, imprecise exception, unimplemented operation
            - data store interrupt caused by dcbf/icbi
            ...
            - uncorrected, uncorrectable, unrecoverable memory/storage/system failure
            - power, fan, thermal, temperature, hardware, or link failure causing unrecovered system impact
            
            If ambiguous, return 1 only for explicit unrecovered, persistent, communication-breaking, mount-failing, kernel-terminating, node-crashing, or job-killing failure. Otherwise return 0.
    ...
    public static List<PromptSpec> bglPromptExperiments() {
        return List.of(
                new PromptSpec(
                        ...
                        "BGL_TEMPLATE_AWARE_FINAL_V10_FAST_CACHE_VALIDATED",
