...
public class BglTemplateValidationService {
    ...
    private static final Pattern CLEAR_ANOMALY_SIGNALS = pattern(
            "ciod:\\s+Error\\s+creating\\s+node\\s+map\\s+.*Cannot\\s+allocate\\s+memory"
                    ...
                    + "|mmcs_server\\s+exited\\s+normally\\s+with\\s+exit\\s+code\\s+(?:-[1-9]\\d*|[1-9]\\d*)"
    ...
    private static final Pattern CLEAR_NORMAL_SIGNALS = pattern(
            "^\\s*machine\\s+check:\\s+i-fetch\\.*\\s*(?:0|<ZERO>)\\s*$"
                    ...
                    + "|^\\s*(?:uncorrectable\\s+error.*|memory\\s+manager\\s+uncorrectable\\s+error)\\.*\\s*(?:0|<ZERO>)\\s*$"
    ...
    public BglTemplateValidationResult validateForCache(
        ...
        boolean anomalySignal = contains(CLEAR_ANOMALY_SIGNALS, raw, normalized);
        boolean normalSignal = contains(CLEAR_NORMAL_SIGNALS, raw, normalized);

        if (anomalySignal && normalSignal) {
            return BglTemplateValidationResult.suspicious("Conflicting exact signals; result was not cached.");
        }
        if (prediction == ClassificationResult.NORMAL && anomalySignal) {
            return BglTemplateValidationResult.suspicious("NORMAL conflicts with an exact anomaly pattern; result was not cached.");
        }
        if (prediction == ClassificationResult.ANOMALY && normalSignal) {
            return BglTemplateValidationResult.suspicious("ANOMALY conflicts with an exact normal pattern; result was not cached.");
        }

        return BglTemplateValidationResult.approved("No exact cache conflict detected.");
    }
    ...
    private static boolean contains(Pattern pattern, String... values) {
        for (String value : values) {
            if (value != null && !value.isBlank() && pattern.matcher(value).find()) {
                return true;
            }
        }
        return false;
